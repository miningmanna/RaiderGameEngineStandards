...
public class LwsFileData {
	...
	public ObjectKeyFrames[] frames;
	public ObjectAlphaKeyFrames[] alphaFrames;
	public int[] parent;
	...
	public static LwsFileData getLwsFileData(InputStream in) throws IOException {
		...
		HashMap<String, Integer> nodeNameCounter= new HashMap<>();
		...
		LinkedList<String> _nodeNames = new LinkedList<>();
...
//				frames = res.lastFrame - res.firstFrame;
			...
			if(line.startsWith("AddNullObject") || line.startsWith("LoadObject")) {
				...
				int index = 0;
				if(nodeNameCounter.containsKey(line))
					index = nodeNameCounter.get(line);
				nodeNameCounter.put(line, index+1);
				...
				of.transforms = new Matrix4f[lframes];
		...
		System.out.println("LOBJECTS: " + lobjects);
		
		res.objFiles = new String[lobjects];
		res.nodeNames = new String[lobjects];
		res.parent = new int[lobjects];
		res.frames = new ObjectKeyFrames[lobjects];
		res.alphaFrames = new ObjectAlphaKeyFrames[lobjects];
//		Vector3f[] objectPivot = new Vector3f[lobjects];
		for(int i = 0; i < lobjects; i++) {
			res.objFiles[i] = _objFiles.pop();
			res.nodeNames[i] = _nodeNames.pop();
			res.parent[i] = _objParent.pop();
			res.frames[i] = _objKeyFrames.pop();
			res.alphaFrames[i] = _objAlphaFrames.pop();
			res.frames[i].pivot = _objPivot.pop();
			res.frames[i].genTransforms();
		}
...
//	}
	...
	public static class ObjectKeyFrames {
		public int lframes;
		public int[] frames;
...
//				m.scale(scales[i]);
				m.rotateAround(rot, pivot.x, pivot.y, pivot.z);
//				m.origin(new Vector3f(0));
	...
	public static class ObjectAlphaKeyFrames {
		public int lframes;
		public int[] frames;
		public float[] alpha;
	}
